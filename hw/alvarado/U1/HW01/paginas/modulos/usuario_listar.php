<?php 
// Seguridad
if (!in_array('usuario_listar', $_SESSION['permisos'])) {
...
/* ================= TOTAL ================= */
$sql_total = "SELECT COUNT(*) AS total FROM usuarios u $where_sql";
...
$where_sql
ORDER BY u.Estado_user = 'Activo' DESC
LIMIT ?, ?
";
...
<form method="get" action="inicio.php" class="w3-center w3-margin-bottom">
    <input type="hidden" name="op" value="usuario_listar">
...
<button class="w3-button w3-blue" type="submit" name="buscar" value="1">Filtrar</button>
<a class="w3-button w3-light-grey" href="inicio.php?op=usuario_listar">Limpiar</a>
...
<?php else: ?>
<tr>
    <td colspan="9" class="w3-center">No hay registros</td>
</tr>
<?php endif; ?>
</table>
...
<?php if ($pagina > 1): ?>
    <a class="w3-button w3-light-grey"
       href="inicio.php?op=usuario_listar&pagina=<?= $pagina - 1 ?><?= $parametros_url ?>">Anterior</a>
<?php else: ?>
...
<?php for ($i = 1; $i <= $total_paginas; $i++): ?>
    ...
    <?php else: ?>
        <a class="w3-button w3-light-grey"
           href="inicio.php?op=usuario_listar&pagina=<?= $i ?><?= $parametros_url ?>"><?= $i ?></a>
    <?php endif; ?>
<?php endfor; ?>

<?php if ($pagina < $total_paginas): ?>
    <a class="w3-button w3-light-grey"
       href="inicio.php?op=usuario_listar&pagina=<?= $pagina + 1 ?><?= $parametros_url ?>">Siguiente</a>
<?php else: ?>
