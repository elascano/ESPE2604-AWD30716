...
<form method="get" action="inicio.php" class="w3-center w3-margin-bottom">
    <input type="hidden" name="op" value="rol_editar_buscar">
    ...
    <a class="w3-button w3-light-grey" href="inicio.php?op=rol_editar_buscar">
        Limpiar
    </a>
</form>
...
<tr>
	...
	<td>
	<?php if ($fila['Estado_rol'] === 'Activo'): ?>
		<form action="../server/cambiar_estado_rol.php"
			  method="post"
			  onsubmit="return confirm('¿Está seguro de desactivar este rol?');">
			<input type="hidden" name="id_rol" value="<?= $fila['id_rol'] ?>">
			<input type="hidden" name="Estado_rol" value="Inactivo">
			<button class="w3-button w3-red" type="submit">ELIMINAR</button>
		</form>
	<?php else: ?>
		<form action="../server/cambiar_estado_rol.php" method="post">
			<input type="hidden" name="id_rol" value="<?= $fila['id_rol'] ?>">
			<input type="hidden" name="Estado_rol" value="Activo">
			<button class="w3-button w3-green" type="submit">ACTIVAR</button>
		</form>
	<?php endif; ?>
	</td>
</tr>
...
<?php if ($pagina > 1): ?>
    <a class="w3-button w3-light-grey"
       href="inicio.php?op=rol_editar_buscar&pagina=<?= $pagina - 1 ?><?= $parametros_url ?>">
       Anterior
    </a>
<?php else: ?>
...
<?php for ($i = 1; $i <= $total_paginas; $i++): ?>
    ...
    <?php else: ?>
        <a class="w3-button w3-light-grey"
           href="inicio.php?op=rol_editar_buscar&pagina=<?= $i ?><?= $parametros_url ?>">
           <?= $i ?>
        </a>
    <?php endif; ?>
<?php endfor; ?>
