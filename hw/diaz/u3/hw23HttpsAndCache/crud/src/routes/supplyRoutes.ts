...
const SUPPLIES_CACHE_KEY = 'supplies:all';

supplyRouter.get('/supplies', async (req: Request, res: Response) => {
  try {
    const cached = getFromCache(SUPPLIES_CACHE_KEY);
    if (cached) {
    ...
    setInCache(SUPPLIES_CACHE_KEY, inventory);
    res.set('Cache-Control', 'max-age=60');
...
supplyRouter.post('/supplies', async (req: Request, res: Response) => {
  try {
    ...
    invalidateCache(SUPPLIES_CACHE_KEY);
...
supplyRouter.put('/supplies/:id', async (req: Request, res: Response) => {
  try {
    ...
    const updateData: Prisma.suppliesUpdateInput = {};
