...
const error = ref('');
const result = ref<{
  pediatricCategory: string;
  calculatedAgeYears: number;
  calculatedAgeMonths: number;
  dosageRestrictionFactor: number;
} | null>(null);
...
async function run() {
  ...
  result.value = null;
  try {
    const data = await apiRequest('/fabuladental/patients/pediatric-category', {
      method: 'POST',
      body: {
  ...
  <div class="view-container">
    <header class="view-header">
      <div>
        ...
        <h1>Pediatric Category</h1>
      </div>
    </header>
    <p class="description">
      Classifies a patient by age group and determines the clinical dosage restriction factor based on their date of birth.
    </p>
    <form @submit.prevent="run" class="rule-form">
      <div class="field">
        <label class="field-label" for="pediatric-birthday">Date of birth</label>
        <input
    ...
    <div v-if="result" class="rule-result">
      ...
      <div class="result-row">
        <span class="result-key">Pediatric Category</span>
        <span class="result-val accent">{{ result.pediatricCategory }}</span>
      </div>

      <div class="result-row">
        <span class="result-key">Calculated Age</span>
        <span class="result-val">{{ result.calculatedAgeYears }} years, {{ result.calculatedAgeMonths }} months</span>
      </div>

      <div class="result-row">
        <span class="result-key">Dosage Restriction Factor</span>
        <span class="result-val accent">{{ result.dosageRestrictionFactor }}</span>
      </div>
    </div>
  </div>
</template>
