...
const result = ref<{
  ...
  suggestedDiscountFactor: number;
} | null>(null);
...
async function run() {
  ...
  result.value = null;
  try {
    const data = await apiRequest('/fabuladental/patients/senior-discount', {
      method: 'POST',
      body: {
...
<template>
  <div class="view-container">
    <header class="view-header">
      <div>
        <p class="eyebrow">Billing Rules</p>
        <h1>Senior Discount</h1>
      </div>
    </header>
    <p class="description">
      Determines whether a patient qualifies for the senior citizen discount (50%) based on their date of birth. Eligibility threshold is 65 years old.
    </p>
    <form @submit.prevent="run" class="rule-form">
      <div class="field">
        <label class="field-label" for="senior-birthday">Date of birth</label>
        <input
          id="senior-birthday"
          type="date"
          v-model="birthday"
    ...
    <div v-if="result" class="rule-result">
      ...
      <div class="result-row">
        <span class="result-key">Eligibility Status</span>
        <span v-if="result.isEligibleForDiscount" class="result-val accent">Eligible — 50% discount</span>
        <span v-else class="result-val muted">Not eligible</span>
      </div>

      <div class="result-row">
        <span class="result-key">Suggested Discount Factor</span>
        <span class="result-val">{{ formatPercent(result.suggestedDiscountFactor) }}</span>
      </div>
    </div>
  </div>
</template>
...
.result-val.muted { color: var(--color-text-muted); }
