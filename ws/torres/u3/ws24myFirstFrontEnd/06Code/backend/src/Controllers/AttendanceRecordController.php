...
final class AttendanceRecordController
{
    ...
    public function index(Request $request, Response $response): Response
    {
        ...
        if ($authUser->role() === 'teacher') {
            $query->where('person_type', 'teacher')->where('person_name', $authUser->name());
        } else {
        ...
        return $this->responder->json($response, [
            'month' => $range->month(),
            'teacher_payroll' => $this->payroll->summarize($records->filter(fn($r) => $r->person_type === 'teacher')),
            'data' => $records,
        ]);
    }
    ...
    public function store(Request $request, Response $response): Response
    {
        ...
        $errors = $this->validator->validateAttendanceManual($data);
        if ($errors !== []) {
        ...
        $this->audit->record($authUser, 'attendance_record.created', 'attendance_records', (int) $attendance->id, [
            'branch_id' => (int) $data['branch_id'], 'status' => $attendance->status, 'source' => $attendance->source,
        ]);

        return $this->responder->json($response, ['message' => 'Attendance registered.', 'data' => $attendance], 201);
    }
}
