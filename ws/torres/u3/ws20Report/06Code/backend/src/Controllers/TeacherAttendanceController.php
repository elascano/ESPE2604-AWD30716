...
final class TeacherAttendanceController
{
    ...
    public function store(Request $request, Response $response): Response
    {
        ...
        $errors = $this->validator->validateAttendanceTeacherKiosk($data);
        if ($errors !== []) {
        ...
        $attendance = AttendanceRecord::query()->create([
            'branch_id' => $branchId, 'student_id' => null, 'national_id' => null,
            'person_type' => 'teacher', 'person_name' => $teacher->name,
            'level' => trim((string) ($data['style'] ?? '')),
            'attendance_date' => $attendanceDate, 'check_in_at' => $checkIn->format('Y-m-d H:i:s'),
            'expected_start_time' => $data['expected_start_time'], 'duration_hours' => $data['duration_hours'],
            'pay_rate' => 12, 'status' => $status, 'source' => 'teacher_kiosk',
